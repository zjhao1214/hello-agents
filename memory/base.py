...
class MemoryConfig(BaseModel):
    ...
    # 统计显示用的基础配置（仅用于展示）
    max_capacity: int = 100  # 最大容量，默认 100
    importance_threshold: float = 0.1 # 重要性阈值，默认 0.1
    decay_factor: float = 0.95 # 衰减因子，默认 0.95
...
class BaseMemory(ABC):
    ...
    def update(self, memory_id: str, content: str = None,
               importance: float = None, metadata: Dict[str, Any] = None) -> None:
        """更新记忆
